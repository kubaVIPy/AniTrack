...
export async function fetchWithRetry(
    url: string,
    options: { retries?: number; retryDelayMs?: number; retryStatuses?: number[] } = {}
): Promise<Response> {
    const { retries = 3, retryDelayMs = 1200, retryStatuses = [429, 503, 504] } = options;
...
export async function fetchUserWatchingListFromMalJson(username: string): Promise<any[]> {
    const encodedUsername = encodeURIComponent(username);
    // Target MAL's internal list endpoint (status=1 means 'watching')
    const targetUrl = `https://myanimelist.net/animelist/${encodedUsername}/load.json?status=1`;

    // Route through corsproxy.io to bypass browser CORS blocks
    ...
    // Map the internal MAL properties to match your app's unified watchlist format
    return rawList.map((item: any) => {
        // Clean up MAL's thumbnail path to get the original high-resolution poster
        let highResPoster = item.anime_image_path || 'https://placehold.co/300x450?text=Poster';
        if (highResPoster.includes('/r/')) {
            // Removes thumbnail scaling path parameters
            highResPoster = highResPoster.replace(/\/r\/\d+x\d+/, '');
            highResPoster = highResPoster.split('?')[0]; // Strip scaling queries
        }
        ...
        return {
            anime: {
                ...
                title_english: item.anime_title, // Fallback to title
                images: {
                    webp: {
                        image_url: highResPoster
                    }
                },
                status: item.anime_airing_status === 1 ? 'Currently Airing' : 'Finished Airing',
...
export function normalizeAnimeStatus(status: string | undefined, fallbackAiring = false): string {
    if (status === 'RELEASING') return 'Currently Airing';
    if (status === 'FINISHED') return 'Finished Airing';
    if (status === 'NOT_YET_RELEASED') return 'TBA';
...
export function buildBroadcastFromAiringAt(airingAt: number) {
    ...
    const localDay = airingDate.toLocaleDateString('en-US', { weekday: 'long', timeZone: localTimeZone });
    ...
    const jstDay = airingDate.toLocaleDateString('en-US', { weekday: 'long', timeZone: 'Asia/Tokyo' });
...
export async function fetchAniListMediaByMalId(malId: number) {
    const response = await fetchGraphqlWithRetry('https://graphql.anilist.co', {
        query: ANILIST_MEDIA_QUERY,
        variables: { idMal: malId }
    }, { retries: 3, retryDelayMs: 1000 });

    if (!response.ok) {
        throw new Error(`AniList lookup failed (${response.status}) for MAL id ${malId}.`);
    }
...
export async function fetchAniListScheduleMap(malIds: number[]) {
    ...
    const mediaList = await Promise.all(uniqueIds.map(async malId => {
        try {
    ...
    mediaList.forEach(media => {
        ...
        scheduleMap[media.idMal] = {
            ...
            broadcast: airingEpisode ? buildBroadcastFromAiringAt(airingEpisode.airingAt) : null,
...
export function getUtcFromJst(year: number, month: number, day: number, hour: number, minute: number): number {
    const pad = (n: number) => String(n).padStart(2, '0');
    return new Date(`${year}-${pad(month)}-${pad(day)}T${pad(hour)}:${pad(minute)}:00+09:00`).getTime();
}
...
export async function fetchUserProfile(username: string) {
    ...
    const fallbackProfile = {
        username: username,
        images: { webp: { image_url: `https://placehold.co/100x100?text=${encodeURIComponent(username.substring(0, 3).toUpperCase())}` } },
        statistics: { anime: { watching: 0 } }
    };

    try {
        const profileRes = await fetchWithRetry(`https://api.jikan.moe/v4/users/${encodedUsername}`, { retries: 2, retryDelayMs: 1000 });
        if (profileRes.ok) {
...
export function parseMalWatchingListMarkdown(markdown: string, username: string): any[] {
    const sectionStart = markdown.indexOf('CURRENTLY WATCHING');
    if (sectionStart === -1) {
        throw new Error('Unable to read the MyAnimeList watching page for this user.');
    }

    const sectionEnd = markdown.indexOf('MyAnimeList.net is a property', sectionStart);
    const section = markdown.slice(sectionStart, sectionEnd === -1 ? undefined : sectionEnd);
    const rowPattern = /\|\s*\|\s*\d+\s*\|\s*\[!\[Image\s*\d+\]\((https?:\/\/cdn\.myanimelist\.net\/[^)]+)\)\]\((?:https?:\/\/)?myanimelist\.net\/anime\/(\d+)\/[^)]+\)\s*\|\s*\[([^\]]+)\]\((?:https?:\/\/)?myanimelist\.net\/anime\/\2\/[^)]+\)([\s\S]*?)\|\s*\[([\-\d]+)\]\([^)]+\)\s*\|\s*([A-Za-z ]+)\s*\|\s*\[([\-\d]+)\]\([^)]+\)\s*\/\s*([\-\d]+|-)\s*\|/g;
    ...
    if (listItems.length === 0) {
        throw new Error(`No anime entries were found on the MyAnimeList watching page for ${username}.`);
    }

    return listItems;
}

export async function fetchUserWatchingListFromMalPage(username: string): Promise<any[]> {
    const encodedUsername = encodeURIComponent(username);
    const mirroredUrl = `https://r.jina.ai/http://myanimelist.net/animelist/${encodedUsername}?status=1&order=12&order2=0`;
    const pageRes = await fetchWithRetry(mirroredUrl, { retries: 2, retryDelayMs: 1000 });

    if (!pageRes.ok) {
        throw new Error(`Unable to read MyAnimeList list page (${pageRes.status}).`);
    }

    const pageText = await pageRes.text();
    return parseMalWatchingListMarkdown(pageText, username);
}

export async function fetchUserWatchingListFromJikan(username: string): Promise<any[]> {
    const encodedUsername = encodeURIComponent(username);
    const animeListRes = await fetchWithRetry(`https://api.jikan.moe/v4/users/${encodedUsername}/animelist?status=watching&limit=100`, { retries: 3, retryDelayMs: 1500 });
    if (!animeListRes.ok) {
        const err: any = new Error(`MyAnimeList API error (${animeListRes.status})`);
        err.code = animeListRes.status === 404 ? 'NOT_FOUND' : 'TEMPORARY_UNAVAILABLE';
        throw err;
    }
...
export async function fetchUserWatchingList(username: string): Promise<any[]> {
    try {
        // 1. Try direct MAL JSON list via CORS proxy (fastest and most stable)
        console.log('Fetching via direct MAL JSON proxy...');
        return await fetchUserWatchingListFromMalJson(username);
    } catch (proxyError) {
        console.warn('Direct MAL JSON fetch failed, falling back to Jican API...', proxyError);
    }

    // 2. Fall back to Jikan API if the proxy is busy
    return fetchUserWatchingListFromJikan(username);
}

export async function fetchLiveWeeklySchedule(): Promise<any[]> {
    try {
        const scheduleRes = await fetch(`https://api.jikan.moe/v4/schedules?limit=100`);
        if (scheduleRes.ok) {
...
export function mapJikanShowsToWatchlist(rawShows: any[]): any[] {
    return rawShows.map(show => {
        ...
        return {
            ...
            image_url: show.images?.webp?.large_image_url || show.images?.webp?.image_url || 'https://placehold.co/300x450?text=Poster',
            ...
            genres: show.genres || [],
            season: show.season ? `${show.season.charAt(0).toUpperCase() + show.season.slice(1)} ${show.year || ''}` : 'Airing',
            url: show.url
        };
    });
}
...
export function getLocalAiringTime(broadcast: any): string {
    if (broadcast?.airingAt) {
        return new Date(broadcast.airingAt * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    }
    if (!broadcast || !broadcast.day || !broadcast.time) return 'TBA';
    const targetUtcEpoch = getJstOccurrenceUtc(broadcast);
    if (targetUtcEpoch == null) return broadcast.time;
    return new Date(targetUtcEpoch).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
}
