...
function initApp() {
    ...
    elements.malForm?.addEventListener('submit', (e) => {
        e.preventDefault();
        const inputVal = (elements.usernameInput as HTMLInputElement | null)?.value.trim();
        if (inputVal) {
...
async function loadUserProfileAndWatchlist(username: string) {
    ...
    try {
        ...
        const anilistScheduleMap = await fetchAniListScheduleMap(malList.map(item => item.anime?.mal_id));

        const enrichedList: any[] = [];
        malList.forEach(item => {
            const baseAnime = item.anime;
            const cachedProgress = getLocalProgress(baseAnime.mal_id, item.episodes_watched);
            ...
            enrichedList.push({
                ...
                image_url: baseAnime.images?.webp?.large_image_url || baseAnime.images?.webp?.image_url || '',
                ...
                genres: genres,
                season: baseAnime.season ? `${baseAnime.season.charAt(0).toUpperCase() + baseAnime.season.slice(1)} ${baseAnime.year || ''}` : 'TBA',
                url: baseAnime.url,
...
function getReleasedEpisodes(anime: any): number {
    ...
    const watchedFallback = Math.max(Number(anime.episodes_watched_mal) || 0, Number(anime.episodes_watched_local) || 0, 0);
...
function getBehindIndicatorData(anime: any) {
    ...
    if (behindEpisodes <= 0) {
        return { text: '✅ Up to date', className: 'text-[10px] font-extrabold text-emerald-400' };
    }
    return { text: `⚠️ ${behindEpisodes} ep behind`, className: 'text-[10px] font-extrabold text-rose-400' };
}

function updateBehindSummary(list = animeWatchlist) {
    if (!elements.totalBehindCount) return;
    const totalBehind = list.reduce((sum, anime) => sum + getBehindEpisodes(anime), 0);
...
function renderCalendar() {
    ...
    weekDaysOrdered.forEach((dayNum) => {
        ...
        const airingTodayAnime = animeWatchlist.filter(anime => {
            if (!anime.airing || !anime.broadcast || !anime.broadcast.day) return false;
...
function renderWatchlist() {
    ...
    filteredList.forEach(anime => {
        ...
        card.className = "flex flex-col rounded-2xl border border-darkBorder bg-darkCard/50 backdrop-blur-md overflow-hidden hover:border-brand-500/30 hover:shadow-lg transition-all duration-300 relative group";
        ...
        const hasBroadcast = anime.broadcast && anime.broadcast.string && !anime.broadcast.string.toLowerCase().includes('tba');
        ...
        if (anime.airing && anime.broadcast?.day) {
            ...
            const genresHtml = anime.genres && anime.genres.length > 0
                ? `<div class="flex flex-wrap gap-1 mt-1">${anime.genres.slice(0, 2).map((g: any) => `<span class="text-[9px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded-md font-semibold">${escapeHtml(g.name)}</span>`).join('')}</div>`
                : '';
            const studioText = anime.studio && anime.studio !== 'Unknown Studio' ? escapeHtml(anime.studio) : 'TBA';
...
export function incrementProgress(malId: number, totalEpisodes: number) {
    ...
    if (barEl) {
        const percentage = totalEpisodes > 0 ? Math.min((currentWatched / totalEpisodes) * 100, 100) : 0;
...
function showMiniToast(message: string) {
    ...
    toast.className = "fixed bottom-5 right-5 bg-gradient-to-r from-brand-600 to-indigo-600 text-white font-semibold text-xs py-3 px-5 rounded-xl shadow-2xl z-50 border border-brand-500/30 transition-all duration-300 transform translate-y-10 opacity-0";
...
export function openAnimeDetails(malId: number) {
    ...
    if (elements.modalPoster) (elements.modalPoster as HTMLImageElement).src = anime.image_url;
    if (elements.modalTitle) elements.modalTitle.textContent = anime.title;
    if (elements.modalTitleEnglish) elements.modalTitleEnglish.textContent = anime.title_english !== anime.title ? anime.title_english : '';
    if (elements.modalScore) elements.modalScore.textContent = anime.score ? anime.score.toFixed(2) : 'N/A';
    if (elements.modalBroadcast) elements.modalBroadcast.textContent = anime.broadcast?.string || 'TBA';
    if (elements.modalStudio) elements.modalStudio.textContent = anime.studio;
    if (elements.modalEpisodes) elements.modalEpisodes.textContent = `${anime.episodes_total || '∞'} episodes`;
    if (elements.modalSeason) elements.modalSeason.textContent = anime.season;
    if (elements.modalSynopsis) elements.modalSynopsis.textContent = anime.synopsis || "No synopsis available.";
